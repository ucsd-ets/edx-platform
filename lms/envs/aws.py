...
# 'caliper_tracking' app allows us to transform Edx event-logs according to
...
#
# Following are the changes made in Django settings by caliper_tracking app.

# 1. EVENT_TRACKING_BACKENDS settings:
#      It is responsible for logging events of 'eventtracking' app, our
#      'caliper_tracking' app appends its custom processor
#      ('caliper_tracking.processor.CaliperProcessor') in the pipeline of
...
#      'caliper_tracking' replaces the default logging backend with its
#      own custom backend which eventually redirects the logs to
#      caliper_tracking's processor.
#
#      TRACKING_BACKENDS = {
#        'logger': {
