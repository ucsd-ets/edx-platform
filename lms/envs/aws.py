...
#
# Following are the changes made on edx by caliper_tracking app.

# 1. EVENT_TRACKING_BACKENDS settings:
#      It is responsible for logging events of 'eventtracking' app, our
#      'caliper_tracking' app appends its custom processor (named
#      'caliper_tracking.processor.CaliperProcessor') in the pipeline of
#      'processors' present in 'tracking_logs' of 'EVENT_TRACKING_BACKENDS'.
#
# 2. TRACKING_BACKENDS settings:
#      Logs generated by 'track' app use 'TRACKING_BACKENDS' settings.
#      'caliper_tracking' replaces its processor with its own custom backend
#      (named 'CALIPER_TRACKING_BACKENDS') which eventually redirects the logs
#      to caliper's processor.
#
